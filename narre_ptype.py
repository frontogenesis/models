"""Plots precipitation type from user-specified weather prediction model"""
...
##############################################
    ...
    def mapFigure(title, fname):
        plt.title(title + '\n' + validtimes[i] + '')
        plt.savefig(fname + str(i) + '.png', dpi=300, bbox_inches='tight')
        print('Finished creating image', i)

    mapFigure('Precipitation Type', 'ptype')

file.close()
print('The program has complete.')
